class WarehousesController < ApplicationController
  ...
  def destroy
    ...
    ActiveRecord::Base.transaction do
      @entrada_almacen.warehouse_details.map do |detail|
        ...
        prod_registrado.existence-= detail.cantidad
  ...
  def add_item
    ...
    existencia_antes_entrada = producto.existence

    result = {
      product_id:      @detalle_entrada.product_id,
      producto:        @detalle_entrada.product.try(:name),
      cantidad:        @detalle_entrada.cantidad
    }

    producto.existence  = existencia_antes_entrada + cantidad
  ...
  def add_proveedor
    ...
    if proveedor.present?
      ...
      if @entrada_almacen.valid?
        result = { nombre_proveedor: @entrada_almacen.supplier.try(:name) }
